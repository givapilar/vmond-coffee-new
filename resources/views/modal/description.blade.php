...
<!-- Main modal -->
<div id="description-modal-resto{{ $resto->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full">
    <div class="relative w-full h-full max-h-full">
        ...
        <div class="relative bg-white shadow h-full dark:bg-gray-700">
            ...
            <div class="flex items-start justify-between p-4 border-b dark:border-gray-600">
                ...
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="description-modal-resto{{ $resto->id }}">
            ...
            <div class="p-6 space-y-6 h-full max-h-[77%] overflow-y-scroll">
                <div class="grid mb-4 border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 md:grid-cols-2">
                    @if ($resto->image != null)
                        <img style="width: 50%;" class="block mx-auto" src="{{ $global_url_image.$resto->image ?? 'https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=870&q=80' }}" alt="." class="object-cover object-center h-full w-full">
