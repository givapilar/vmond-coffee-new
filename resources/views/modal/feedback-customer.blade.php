...
<!-- Main modal -->
<div id="feedback-modal{{ $item->code }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full overflow-x-hidden overflow-y-auto md:inset-0 h-full max-h-full">
    <!-- Modal content -->
    <div class="relative flex flex-col justify-center item-center  bg-gray-700  w-full h-full max-h-full my-auto">
        <div class="p-5 space-y-4 h-full max-h-[77%] my-auto">
            <div class="max-w-[360px] mx-auto">
                <div class="bg-white shadow-lg rounded-lg mt-9">
                    ...
                    <header class="text-center px-5 pb-5">
                        ...
                        <!-- Card name -->
                        <h3 class="text-xl font-bold text-gray-900 mb-1">Feedback Customer</h3>
                        <div class="text-sm font-medium text-gray-500">Silakan berikan penilaian tentang layanan yang kami sediakan di Vmond Coffee & Eatery Space.</div>
                    </header>
                    <!-- Card body -->
                    <div class="bg-gray-100 text-center px-5 py-6">
                        <form class="space-y-3" method="POST" action="{{ route('feedback', ['token' => Crypt::encryptString('feedback-'.Auth::user()->id.'-customer'), 'id' => Crypt::encryptString('feedback-'.$item->id.'-customer')]) }}">
                            @csrf
                            <div class="flex shadow-sm rounded">
                                <select id="category" class="text-sm text-gray-800 bg-white rounded-l leading-5 py-2 px-3 placeholder-gray-400 w-full border border-transparent focus:border-indigo-300 focus:ring-0" name="feedback">
                            ...
                            <button type="button" class="font-semibold text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow transition duration-150 ease-in-out w-full bg-red-500 hover:bg-red-600 text-white focus:outline-none focus-visible:ring-2" data-modal-target="history-detail-modal{{ $item->invoice_no }}"  data-modal-toggle="history-detail-modal{{ $item->invoice_no }}" data-modal-hide="feedback-modal{{ $item->code }}">Close</button>
                        </form>
                    </div>
                </div>
            </div>
