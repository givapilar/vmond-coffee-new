                ...
                <div class="bg-white shadow-lg rounded-lg mt-9">
                    ...
                    <div class="bg-gray-100 text-center px-5 py-6">
                        <form class="space-y-3" method="POST" action="{{ route('feedback', ['token' => Crypt::encryptString('feedback-'.Auth::user()->id.'-customer'), 'id' => Crypt::encryptString('feedback-'.$item->id.'-customer')]) }}">
                            ...
                            <button type="button" class="font-semibold text-sm inline-flex items-center justify-center px-3 py-2 border border-transparent rounded leading-5 shadow transition duration-150 ease-in-out w-full bg-red-500 hover:bg-red-600 text-white focus:outline-none focus-visible:ring-2" data-modal-target="history-detail-modal{{ $item->invoice_no }}"  data-modal-toggle="history-detail-modal{{ $item->invoice_no }}">Close</button>
                        </form>
                    </div>
                </div>
            </div>
