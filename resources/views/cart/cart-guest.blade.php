                    ...
                    <li class="py-3 sm:py-4">
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div class="rounded-full h-7 w-32 border border-gray-500 mt-2">
                                    <div class="grid h-full w-full grid-cols-3 mx-auto">
                                        <button type="button" class="inline-flex flex-col items-center justify-center px-5 rounded-l-full hover:bg-gray-50 dark:hover:bg-gray-800 group" onclick="remove('{{ strtolower(str_replace(' ','-',$item->attributes['restaurant']['nama'])) }}',  {{ $item->attributes['restaurant']['id'] }})">
                                        ...
                                        <div>
                                            <input type="number" value="{{ $item->quantity }}" name="qty[]" id="count-items" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm text-center focus:ring-primary-600 focus:border-primary-600 block w-full py-0.5 h-full px-1 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 counter-{{ strtolower(str_replace(' ','-',$item->attributes['restaurant']['nama'])) }}" placeholder="0" required="" readonly>
                                            <input type="hidden" value="{{ $key }}" name="idSession[]">
                                        </div>
                                            
                                        <button type="button" class="inline-flex flex-col items-center justify-center px-5 rounded-r-full hover:bg-gray-50 dark:hover:bg-gray-800 group" onclick="add('{{ strtolower(str_replace(' ','-',$item->attributes['restaurant']['nama'])) }}',  {{ $item->attributes['restaurant']['id'] }})">
                            ...
                            <div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                <button type="button" class="flex justify-center items-center w-10 h-10 rounded-full p-2 bg-red-500 hover:bg-red-600 focus:ring-red-900 focus:ring-4"  data-modal-toggle="deleteModal-{{ $key }}">
                    ...
                    <li class="py-3 sm:py-2" style="display: none;" id="meja-takeaway">
                        <div id="select-input-wrapper">
                            ...
                            <select id="countries" name="meja_restaurant_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                <option required value="{{ $meja_restaurant->id }}" {{ $kodeMeja == $meja_restaurant->kode_meja ? 'selected' : '' }}>{{ $meja_restaurant->nama }}</option>
                                {{-- @endif --}}
                                @endforeach
                            </select>
                        </div>
...
<script>
    ...
    function calculateTotalPrice(dataCart) {
        ...
        for (const key in dataCart) {
            ...
            let slugToFind = nestedObject.name.toLowerCase().replace(/\s+/g, "-");
